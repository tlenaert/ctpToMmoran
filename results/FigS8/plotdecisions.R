...
#read all the files
flist<-c("actionanalytical.csv", "avgaction00001.csv","avgaction0001.csv","avgaction001.csv")
...
plot<-ggplot(data=df, aes(x=fct_inorder(factor(tmp)), y=value, fill=factor(variable))) +
  ...
  labs(title="*k*-levels", x="Analytic (*A*) versus simulation (*\u03BC*)", y = "Frequency", fill="actions(*T*)")+
